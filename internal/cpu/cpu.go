...
func NewCPU(bus *memory.Bus, interrupt *interrupt.InterruptController) *CPU {
	return &CPU{
		Bus:       bus,
		Interrupt: interrupt,
	}
...
func (cpu *CPU) Step() uint64 {
	if (cpu.CPSR&BitI) == 0 && (cpu.Interrupt.IME&1) != 0 && (cpu.Interrupt.IF&cpu.Interrupt.IE) != 0 {
...
func ROR(val uint32, amount uint) uint32 {
	return (val >> amount) | (val << (32 - amount))
}

func (cpu *CPU) executeBranchExchange(opcode uint32) uint64 {
	Rn := int(opcode & 0xF)
	val := cpu.ReadReg(Rn)
	isThumb := (val & 1) != 0
	if isThumb {
...
func (cpu *CPU) executeBlockDataTransfer(opcode uint32) uint64 {
	P := (opcode & (1 << 24)) != 0
	U := (opcode & (1 << 23)) != 0
	S := (opcode & (1 << 22)) != 0
	W := (opcode & (1 << 21)) != 0
	...
	if L {
		if (Rlist & (1 << 15)) != 0 {
			return uint64(count) + 4 // (n+1)S + 2N + 1I
		} else {
			return uint64(count) + 2 // nS + 1N + 1I
		}
	} else {
		return uint64(count) + 1 // (n-1)S + 2N
	}
}

func (cpu *CPU) executeBranchAndBranchWithLink(opcode uint32) uint64 {
	pc := cpu.ReadReg(15) // PC+8
...
func (cpu *CPU) executeSoftwareInterrupt() uint64 {
	cpu.HandleException(ExceptSoftwareInterrupt)
	return 3 // 2S + 1N
}

func (cpu *CPU) executeUndefined() uint64 {
	cpu.HandleException(ExceptUndefined)
	return 4 // 2S + 1I + 1N
}

func (cpu *CPU) executeSingleDataTransfer(opcode uint32) uint64 {
	I := (opcode & (1 << 25)) != 0
	P := (opcode & (1 << 24)) != 0
	U := (opcode & (1 << 23)) != 0
	B := (opcode & (1 << 22)) != 0
	...
	if I { // shifted register
		...
		offset, _ = Shift(RmVal, shiftType, shiftAmount, false, true)
	} else { // immediate
		offset = opcode & 0xFFF
	}
	...
	if L { // load
		...
		if B {
			val := cpu.ReadReg(Rd)
			cpu.Bus.Write8(addr, byte(val&0xFF))
		} else {
			val := cpu.ReadReg(Rd)
			cpu.Bus.Write32(addr, val)
		}
...
func (cpu *CPU) executeSingleDataSwap(opcode uint32) uint64 {
	B := (opcode & (1 << 22)) != 0
	Rn := int((opcode >> 16) & 0xF)
	Rd := int((opcode >> 12) & 0xF)
	Rm := int(opcode & 0xF)
	...
	if B {
		...
		memVal = cpu.Bus.Read32(addr)
		cpu.Bus.Write32(addr, RmVal)
	}
	cpu.WriteReg(Rd, memVal)
	return 4 // 1S + 2N + 1I
}

func (cpu *CPU) executeMultiply(opcode uint32) uint64 {
	op := (opcode >> 21) & 0xF
	S := (opcode & (1 << 20)) != 0
...
func (cpu *CPU) executeHalfwordDataTransfer(opcode uint32) uint64 {
	P := (opcode & (1 << 24)) != 0
	U := (opcode & (1 << 23)) != 0
	I := (opcode & (1 << 22)) != 0
	W := (opcode & (1 << 21)) != 0
	...
	if L { // load
		...
		switch op {
		case 0x1: // LDRH
			val = uint32(cpu.Bus.Read16(addr))
		case 0x2: // LDRSB
		...
		RdVal := cpu.ReadReg(Rd)
		cpu.Bus.Write16(addr, uint16(RdVal&0xFFFF))
	}

	if W || !P {
		if U {
			cpu.WriteReg(Rd, RnVal+offset)
		} else {
			cpu.WriteReg(Rd, RnVal-offset)
		}
	}
...
func (cpu *CPU) executePSRTransferMRS(opcode uint32) uint64 {
	src := (opcode & (1 << 22)) != 0
	Rd := int((opcode >> 12) & 0xF)
	var psr uint32
	if src {
...
func (cpu *CPU) executePSRTransferMSR(opcode uint32) uint64 {
	I := (opcode & (1 << 25)) != 0
	dst := (opcode & (1 << 22)) != 0
	var mask uint32
	if (opcode & (1 << 19)) != 0 {
...
func (cpu *CPU) executeDataProcessing(opcode uint32) uint64 {
	I := (opcode & (1 << 25)) != 0
	op := (opcode >> 21) & 0xF
	S := (opcode & (1 << 20)) != 0
	Rn := int((opcode >> 16) & 0xF)
	...
	if I { // immediate
		...
		if R { // register
			Rs := int((opcode >> 8) & 0xF)
			shiftAmount = uint(cpu.ReadReg(Rs) & 0x1F)
			cycles += 1 // +1I
		} else { // immediate
			shiftAmount = uint((opcode >> 7) & 0x1F)
		}
...
func (cpu *CPU) checkCondition(opcode uint32) bool {
	cond := (opcode >> 28) & 0xF
	flagN := (cpu.CPSR & BitN) != 0
	flagZ := (cpu.CPSR & BitZ) != 0
	flagC := (cpu.CPSR & BitC) != 0
	flagV := (cpu.CPSR & BitV) != 0
	switch cond {
	case 0x0: // EQ
		return flagZ
	case 0x1: // NE
...
func (cpu *CPU) ExecuteARM(opcode uint32) uint64 {
	if !cpu.checkCondition(opcode) {
		return 1 // 1S
	}
	switch {
	case IsBranchExchange(opcode):
		return cpu.executeBranchExchange(opcode)
	case IsBlockDataTransfer(opcode):
		return cpu.executeBlockDataTransfer(opcode)
	case IsBranchAndBranchWithLink(opcode):
		return cpu.executeBranchAndBranchWithLink(opcode)
	case IsSoftwareInterrupt(opcode):
		return cpu.executeSoftwareInterrupt()
	case IsUndefined(opcode):
		return cpu.executeUndefined()
	case IsSingleDataTransfer(opcode):
		return cpu.executeSingleDataTransfer(opcode)
	case IsSingleDataSwap(opcode):
		return cpu.executeSingleDataSwap(opcode)
	case IsMultiply(opcode) || IsMultiplyLong(opcode):
		return cpu.executeMultiply(opcode)
	case IsHalfwordDataTransferRegister(opcode):
		return cpu.executeHalfwordDataTransfer(opcode)
	case IsHalfwordDataTransferImmediate(opcode):
		return cpu.executeHalfwordDataTransfer(opcode)
	case IsPSRTransferMRS(opcode):
		return cpu.executePSRTransferMRS(opcode)
	case IsPSRTransferMSR(opcode):
		return cpu.executePSRTransferMSR(opcode)
	case IsDataProcessing(opcode):
		return cpu.executeDataProcessing(opcode)
	}
	// Unknown opcode
	return 1
}
...
func (cpu *CPU) executeThumbSoftwareInterrupt() uint64 {
	cpu.HandleException(ExceptSoftwareInterrupt)
	return 3 // 2S + 1N
}

func (cpu *CPU) executeThumbUnconditionalBranch(opcode uint16) uint64 {
...
func (cpu *CPU) executeThumbConditionalBranch(opcode uint16) uint64 {
	cond := (opcode >> 8) & 0xF
...
func (cpu *CPU) executeThumbMultipleLoadStore(opcode uint16) uint64 {
	op := int((opcode >> 11) & 1)
	Rb := int((opcode >> 8) & 0x7)
	Rlist := opcode & 0xFF
...
func (cpu *CPU) executeThumbLongBranchWithLink(opcode uint16) uint64 {
	H := (opcode >> 11) & 1
	offset := uint32(opcode) & 0x7FF
	...
	if H == 0 {
		...
		cpu.WriteReg(15, cpu.ReadReg(14)+(offset<<1))
		cpu.WriteReg(14, retAddr)
		return 3 // 2S + 1N
	}
}

func (cpu *CPU) executeThumbAddOffsetToStackPointer(opcode uint16) uint64 {
	op := int((opcode >> 7) & 1)
...
func (cpu *CPU) executeThumbPushPopRegisters(opcode uint16) uint64 {
	op := int((opcode >> 11) & 1)
	PC_LR := (opcode & (1 << 8)) != 0
	Rlist := opcode & 0xFF
	...
	switch op {
	case 0x0: // PUSH
		// PUSH LR first if specified
		if PC_LR {
			sp -= 4
			cpu.Bus.Write32(sp, cpu.ReadReg(14))
			cycles += 1 // +1S
		}
		// PUSH registers in descending order (R7 to R0)
		for i := 7; i >= 0; i-- {
			if (Rlist & (1 << i)) != 0 {
				sp -= 4
	...
	case 0x1: // POP
		// POP registers in ascending order (R0 to R7)
		for i := 0; i < 8; i++ {
			if (Rlist & (1 << i)) != 0 {
		...
		// POP PC last if specified
		if PC_LR {
			cpu.WriteReg(15, cpu.Bus.Read32(sp))
			sp += 4
			cycles += 3 // +2S + 1N
		}
	}
...
func (cpu *CPU) executeThumbLoadStoreHalfword(opcode uint16) uint64 {
	op := int((opcode >> 11) & 1)
	...
	switch op {
	case 0x0: // STRH
		cpu.Bus.Write16(addr, uint16(cpu.ReadReg(Rd)&0xFFFF))
		cycles = 2 // 2N
	case 0x1: // LDRH
		cpu.WriteReg(Rd, uint32(cpu.Bus.Read16(addr)))
		cycles = 3 // 1S + 1N + 1I
	}
...
func (cpu *CPU) executeThumbSPRelativeLoadStore(opcode uint16) uint64 {
	op := int((opcode >> 11) & 1)
	Rd := int((opcode >> 8) & 0x7)
...
func (cpu *CPU) executeThumbLoadAddress(opcode uint16) uint64 {
	op := int((opcode >> 11) & 1)
	Rd := int((opcode >> 8) & 0x7)
	nn := uint32(opcode&0xFF) << 2
...
func (cpu *CPU) executeThumbLoadStoreWithImmediateOffset(opcode uint16) uint64 {
...
func (cpu *CPU) executeThumbLoadStoreWithRegisterOffset(opcode uint16) uint64 {
...
func (cpu *CPU) executeThumbLoadStoreSignExtendedByteHalfword(opcode uint16) uint64 {
	op := int((opcode >> 10) & 0x3)
	Ro := int((opcode >> 6) & 0x7)
	Rb := int((opcode >> 3) & 0x7)
	Rd := int(opcode & 0x7)
	...
	case 0x2: // LDRH
		...
		cycles = 3 // 1S + 1N + 1I
	case 0x3: // LDRSH
		val := int32(int16(cpu.Bus.Read16(addr)))
		cpu.WriteReg(Rd, uint32(val))
		cycles = 3 // 1S + 1N + 1I
	}
...
func (cpu *CPU) executeThumbPCRelativeLoad(opcode uint16) uint64 {
	Rd := int((opcode >> 8) & 0x7)
	nn := (uint32(opcode) & 0xFF) << 2

	pc := cpu.ReadReg(15) & 0xFFFFFFFD
...
func (cpu *CPU) executeThumbHiRegisterOperationsBranchExchange(opcode uint16) uint64 {
	op := int((opcode >> 8) & 0x3)
	MSBd := (opcode & (1 << 7)) != 0
	MSBs := (opcode & (1 << 6)) != 0
	Rs := int((opcode >> 3) & 0x7)
	...
	case 0x1: // CMP
		...
		signOp1 := (RdVal>>31)&1 != 0
		signOp2 := (RsVal>>31)&1 != 0
		signRes := (result>>31)&1 != 0
		if signOp1 != signOp2 && signOp1 != signRes {
			flags |= BitV
		}
		cpu.CPSR = (cpu.CPSR & 0x0FFFFFFF) | flags
	case 0x2: // MOV
...
func (cpu *CPU) executeThumbALUOperations(opcode uint16) uint64 {
	op := int((opcode >> 6) & 0xF)
	Rs := int((opcode >> 3) & 0x7)
	Rd := int(opcode & 0x7)
	RsVal := cpu.ReadReg(Rs)
	...
	case 0x3: // LSR
		...
		cycles += 1 // +1I
	case 0x4: // ASR
		result := uint32(int32(RdVal) >> (RsVal & 0xFF))
		if (result & (1 << 31)) != 0 {
			flags |= BitN
		}
		if result == 0 {
	...
	case 0x6: // SBC
		...
		return cycles
	case 0x7: // ROR
		result := (RdVal >> RsVal) | (RdVal << (32 - RsVal))
		if (result & (1 << 31)) != 0 {
			flags |= BitN
		}
		if result == 0 {
...
func (cpu *CPU) executeThumbMoveCompareAddSubtractImmediate(opcode uint16) uint64 {
	op := int((opcode >> 11) & 0x3)
	Rd := int((opcode >> 8) & 0x7)
...
func (cpu *CPU) executeThumbAddSubtract(opcode uint16) uint64 {
	op := int((opcode >> 9) & 0x3)
	Rs := int((opcode >> 3) & 0x7)
	Rd := int(opcode & 0x7)
	RsVal := cpu.ReadReg(Rs)
...
func (cpu *CPU) executeThumbMoveShiftedRegister(opcode uint16) uint64 {
	op := int((opcode >> 11) & 0x3)
	...
	result, carry := Shift(RsVal, op, offset, false, true)
	cpu.WriteReg(Rd, result)
...
func (cpu *CPU) ExecuteThumb(opcode uint16) uint64 {
	switch {
	case IsThumbSoftwareInterrupt(opcode):
		return cpu.executeThumbSoftwareInterrupt()
	case IsThumbUnconditionalBranch(opcode):
		return cpu.executeThumbUnconditionalBranch(opcode)
	case IsThumbConditionalBranch(opcode):
		return cpu.executeThumbConditionalBranch(opcode)
	case IsThumbMultipleLoadStore(opcode):
		return cpu.executeThumbMultipleLoadStore(opcode)
	case IsThumbLongBranchWithLink(opcode):
		return cpu.executeThumbLongBranchWithLink(opcode)
	case IsThumbAddOffsetToStackPointer(opcode):
		return cpu.executeThumbAddOffsetToStackPointer(opcode)
	case IsThumbPushPopRegisters(opcode):
		return cpu.executeThumbPushPopRegisters(opcode)
	case IsThumbLoadStoreHalfword(opcode):
		return cpu.executeThumbLoadStoreHalfword(opcode)
	case IsThumbSPRelativeLoadStore(opcode):
		return cpu.executeThumbSPRelativeLoadStore(opcode)
	case IsThumbLoadAddress(opcode):
		return cpu.executeThumbLoadAddress(opcode)
	case IsThumbLoadStoreWithImmediateOffset(opcode):
		return cpu.executeThumbLoadStoreWithImmediateOffset(opcode)
	case IsThumbLoadStoreWithRegisterOffset(opcode):
		return cpu.executeThumbLoadStoreWithRegisterOffset(opcode)
	case IsThumbLoadStoreSignExtendedByteHalfword(opcode):
		return cpu.executeThumbLoadStoreSignExtendedByteHalfword(opcode)
	case IsThumbPCRelativeLoad(opcode):
		return cpu.executeThumbPCRelativeLoad(opcode)
	case IsThumbHiRegisterOperationsBranchExchange(opcode):
		return cpu.executeThumbHiRegisterOperationsBranchExchange(opcode)
	case IsThumbALUOperations(opcode):
		return cpu.executeThumbALUOperations(opcode)
	case IsThumbMoveCompareAddSubtractImmediate(opcode):
		return cpu.executeThumbMoveCompareAddSubtractImmediate(opcode)
	case IsThumbAddSubtract(opcode):
		return cpu.executeThumbAddSubtract(opcode)
	case IsThumbMoveShiftedRegister(opcode):
		return cpu.executeThumbMoveShiftedRegister(opcode)
	}
	// Unknown
	return 1
}
